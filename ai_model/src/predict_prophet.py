...
parser = argparse.ArgumentParser(description='Generar predicciones de inventario usando Prophet')
...
class ProphetPredictor:
    def __init__(self):
        self.params = ProphetParams()
    
    def fit_predict(self, data: List[float], dates: List[datetime], forecast_periods: int = 6, last_date: datetime = None, dias_operacion: int = 22) -> Dict:
        try:
            logger.debug(f"Ajustando Prophet para datos: {data[:5]}... (len={len(data)})")
            ...
            fitted = forecast['yhat'][:len(data)].tolist()
            
            # Calcular métricas
            errors = np.array(data) - np.array(fitted)
            safe_actual = np.where(np.array(data) == 0, 1e-10, np.array(data))
            metrics = {
            ...
            logger.error(f"Error al ajustar Prophet: {str(e)}")
            avg = np.mean(data) if np.any(data > 0) else 0
            monthly_consumptions = []
            current_date = last_date + relativedelta(months=1)
            for i in range(forecast_periods):
                ...
                monthly_consumptions.append({
                    'month': f"{SPANISH_MONTHS[month_date.month]}-{month_date.year}",
                    'yhat': float(max(0, round(avg, 2)))
                })
            return {
                'monthly_consumptions': monthly_consumptions,
                'fitted': [float(avg)] * len(data),
                'params': self.params.__dict__,
                'metrics': {'mae': 0.0, 'rmse': 0.0, 'mape': 0.0, 'mse': 0.0},
                'seasonality_mode': 'additive'
            }

def generar_predicciones_prophet(series: List[SerieData], ultima_fecha: datetime, forecast_horizon: int = 6, dias_operacion: int = 22) -> Dict:
    resultados = {}
    mape_total = 0.0
    count = 0
    ...
    for serie in series:
        ...
        if not serie.es_valida:
            logger.warning(f"Serie {serie.codigo} no válida, omitiendo Prophet")
            continue
        try:
            ...
            logger.info(f"Producto {serie.codigo} - Parámetros: yearly_seasonality={params['yearly_seasonality']}, seasonality_mode={params['seasonality_mode']}")
...
def cargar_datos():
    try:
        ...
        df["STOCK_INICIAL"] = df["STOCK TOTAL"].astype(float)
        df["STOCK_TOTAL"] = df["STOCK_INICIAL"] + df["UNIDADES EN TRANSITO"].astype(float)
...
def calcular_predicciones(df, cols_consumo, ultima_fecha, fecha_inicio_prediccion, dias_transito, prophet_predictions=None,
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            proyecciones = []
            monthly_consumptions = []
            if prophet_predictions and codigo in prophet_predictions:
                monthly_consumptions = prophet_predictions[codigo]['monthly_consumptions']
                logger.debug(f"Consumos mensuales Prophet para {codigo}: {[mc['yhat'] for mc in monthly_consumptions]}")
            ...
            for mes in range(6):
                ...
                if prophet_predictions and codigo in prophet_predictions:
                    for mc in monthly_consumptions:
                        if mc['month'] == f"{SPANISH_MONTHS[month_date.month]}-{month_date.year}":
                            ...
                            logger.info(f"Producto {codigo}, Mes {month_date.strftime('%Y-%m')}: Predicción Prophet = {consumo_mensual:.2f}")
                            break
                ...
                consumo_leadtime_periodo = consumo_diario * lead_time_days
                ...
                while current_date <= end_date:
                    ...
                    # Calcular stock total proyectado
                    ...
                    if stock_total_proyectado <= punto_reorden:
                        ...
                        fecha_arribo = current_date + timedelta(days=lead_time_days)
                        ...
                        if unidades_a_pedir > 0:
                            ...
                            logger.debug(f"Pedido generado para {codigo} en {fecha_alerta.strftime('%Y-%m-%d')}: {unidades_a_pedir} unidades, llega {fecha_arribo.strftime('%Y-%m-%d')}")
                    ...
                    # Registrar el estado diario después del consumo
            ...
            prophet_info = {}
            if prophet_predictions and codigo in prophet_predictions:
                pred_data = prophet_predictions[codigo]
                prophet_info = {
                    "PARAMETROS": {
                        "yearly_seasonality": pred_data['params']['yearly_seasonality'],
                        "weekly_seasonality": pred_data['params']['weekly_seasonality'],
                        "daily_seasonality": pred_data['params']['daily_seasonality'],
                        "seasonality_mode": pred_data['params']['seasonality_mode'],
                        "changepoint_prior_scale": pred_data['params']['changepoint_prior_scale']
                    },
                    "METRICAS": {
            ...
            producto_info = {
                ...
                "CONFIGURACION": {
                    ...
                    "VERSION_MODELO": "prophet-monthly"
                },
                "PEDIDOS_POR_LLEGAR": [
...
def guardar_resultados(resultados_completos):
    try:
        output_dir = os.path.join(os.path.dirname(__file__), '../data_prophet')
        ...
        for producto in resultados_completos:
            ...
            unidades_por_caja = producto.get('UNIDADES_POR_CAJA', 1.0)
            for proyeccion in producto.get('PROYECCIONES', []):
                # Calcular déficit y pedidos condicionalmente
                stock_total_proyectado = proyeccion.get('stock_total_proyectado', 0.0)
                punto_reorden = proyeccion.get('punto_reorden', 0.0)
                deficit = max(stock_total_proyectado - punto_reorden, 0.0) if stock_total_proyectado < punto_reorden else 0.0
                cajas_a_pedir = int(np.ceil(deficit / unidades_por_caja)) if deficit > 0 else 0
                unidades_a_pedir = float(cajas_a_pedir * unidades_por_caja)
                
                # Manejar alertas_y_pedidos para evitar index out of range
                alertas = proyeccion.get('alertas_y_pedidos', [])
                fecha_reposicion = alertas[0].get('fecha_arribo', '') if alertas else ''
                
                proyecciones_data.append({
                    'Código': codigo,
                    'Descripción': descripcion,
                    'Período': proyeccion.get('mes', ''),
                    'Fecha Solicitud': proyeccion.get('fecha_inicio_mes', ''),
                    'Fecha Fin Período': proyeccion.get('fecha_fin_mes', ''),
                    'Consumo Período': proyeccion.get('consumo_mensual', 0.0),
                    'Consumo Mensual': proyeccion.get('consumo_mensual', 0.0),
                    'Stock Inicial': proyeccion.get('stock_inicial_mes', 0.0),
                    'Stock Proyectado': proyeccion.get('stock_proyectado_mes', 0.0),
                    'Déficit': deficit,
                    'Cajas a Pedir': cajas_a_pedir,
                    'Unidades a Pedir': unidades_a_pedir,
                    'Pedidos Recibidos': proyeccion.get('unidades_en_transito', 0.0),
                    'Alerta Stock': proyeccion.get('indicador_riesgo', 'Bajo') != 'Bajo',
                    'Fecha Reposición': fecha_reposicion,
...
def main():
    try:
        ...
        prophet_predictions = generar_predicciones_prophet(
            series_limpias, 
            ultima_fecha_df,
