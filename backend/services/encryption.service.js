...
class EncryptionService {
  ...
  /**
   * Valida que las llaves RSA se hayan cargado correctamente.
   * @private
   */
  _validateKeys() {
    if (!this.privateKey || !this.publicKey) {
      logger.error(
        "FALLO CRÍTICO: No se pudieron cargar las llaves RSA desde variables de entorno."
      );
      // En un entorno de producción, es crucial detener la aplicación si las llaves no están disponibles.
      process.exit(1);
    }
    logger.info("Llaves de cifrado RSA cargadas exitosamente desde .env.");
  }
  /**
  ...
  getPublicKey() {
    if (!this.publicKey)
      throw new Error("La llave pública no está disponible.");
  ...
  decryptSessionKey(encryptedKeyB64) {
    if (!this.privateKey)
      throw new Error("La llave privada no está disponible.");
    try {
  ...
  _decryptAES(encryptedPayloadB64, aesKeyBase64) {
    try {
      ...
      if (key.length !== 32) {
        throw new Error(
          `Longitud de llave AES inválida: ${key.length} bytes (esperado: 32 bytes)`
        );
      }
